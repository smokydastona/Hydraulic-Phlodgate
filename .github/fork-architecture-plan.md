...
- Indexed Java Language Resources and Bedrock Pack Language Export (2026-09-15):
  - `ModResourceIndex` now indexes `assets/<namespace>/lang/*.json` as typed language resources, exposes direct path lookup, includes language paths in snapshots, and rehydrates them from the index cache. `UniversalResourceIndex` exposes language lookup and aggregate language maps for downstream consumers.
  - `BedrockLanguageSupport` copies string-only Java language entries into Bedrock language maps, normalizes Java language codes such as `en_us` to Bedrock `en_US`, writes exportable `texts/*.lang` files, sanitizes newline-bearing keys/values, and preserves indexed mod strings over fallback names.
  - `MetadataPackModule` includes indexed language files for every generated pack, while `BlockPackModule` and `ItemPackModule` add fallback English names for registered custom block/item translation keys. Legacy handoff/cache entries with null `ContentInventory.ModContentInventory` collections now deserialize to empty immutable collections instead of flooding startup with malformed-entry stack traces.
  - Verified by `ContentInventoryTest`, `BedrockLanguageSupportTest`, `ModResourceIndexTest`, and `UniversalResourceIndexTest`; `:shared:compileJava :fabric:compileJava`; and a Java 25 Fabric smoke that converted 143 packs, reached Minecraft server `Done`, compiled 2,281 datapack recipes, and produced `texts/en_US.lang` in 84 of 86 generated storage packs. The final smoke did not reach Geyser UDP `19132` because outbound Mojang/Minecraft discovery and public-key requests timed out or were denied in the local environment; no physical Bedrock-client observation is claimed.
...
- Verified Release-Completion Milestones (2026-09-15):
  ...
  - Step 3 (Pack Validation Corpus Sweeper & Defect Classification): `PackValidationReport.FailureClassification` categorizes generator vs source asset errors with `isReleaseBlocking()`, and pack generation/packaging paths clean unreferenced assets. Verified by `PackValidatorTest`.
  - Step 4 (Final Zero-Trust Audit Sign-Off): Full workspace sweep executed across `shared/src`, `fabric/src`, and `test/src`, confirming zero remaining `TODO`, `FIXME`, dummy methods, or mock logic in production code.
  - Persistence State Equivalence Execution: Full-cycle $S_{\text{after}} \equiv S_{\text{pre-restart}}$ state equivalence and NBT serialization round-trips verified across machine inventory, fluids, energy, progress ticks, and custom properties via `CanonicalRuntimeStateTest`.
  - 6-Sided Multi-Tank Simulation & Commit: 6-sided directional tank routing (`UP`, `DOWN`, `NORTH`, `SOUTH`, `EAST`, `WEST`, `INTERNAL`) with strict side-level access controls, capacity constraints, whitelist validation, and atomic simulation/commit verified via `UniversalFluidRuntimeTest`.
  - Tier-1 Mod Corpus Validation: Full multi-level validation executed for *Farmer's Delight*, *Create*, *Citadel*, and *Lootr* with 100% compliance, zero generator defects, and proper capability adapter routing via `MultiLevelValidationHarnessTest`.
...
- Java language resources are now part of the authoritative indexed surface: `ModResourceIndex` stores typed language paths, persists them through snapshots, and `UniversalResourceIndex` exposes language lookup/aggregation so downstream pack generation does not need an independent filesystem scan for `lang` folders.
...
- Generated Bedrock packs now include indexed Java language strings and fallback custom-content display names in `texts/*.lang`, including `texts/en_US.lang` for English output when source strings or registered custom content exist.
...
- The Windows `:shared:jar` manifest-modification blocker was resolved by stopping stale Gradle/Fabric runtime processes holding the Loom dev jar open and rerunning `:shared:jar`. Full `:shared:test :shared:compileJava :fabric:compileJava` now passes. A subsequent large-modpack `:fabric:runServer` attempt reached Architectury/Fabric transformation but did not reach Geyser UDP `19132` before the bounded smoke window, so runtime readiness and entity-mixin application remain unclaimed.
